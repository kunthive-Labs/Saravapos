...
Saravapos fills that gap. A profile is a small YAML/JSON document that describes a persona's expertise, preferred analogies, cognitive style, and cultural reference points. Given a `from` profile and a `to` profile, a translator can rewrite content so it lands — preserving the **semantic** payload while swapping the **metaphor** layer.
